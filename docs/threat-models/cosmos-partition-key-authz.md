...
| **Repudiation** | Attacker denies having modified a document; write audit trail is absent. | Cosmos diagnostic settings (`DataPlaneRequests`, `QueryRuntimeStatistics`) forwarded to Log Analytics. `CosmosRequestChargeLogger` emits structured per-op logs (operation, container, partition key, RU charge) on every call. `AuditMiddleware` logs HTTP function start/end; per-handler `AuditLog.Emit` records mutating actions with the actor id HMAC-hashed via `IActorHasher`. `Audit__HashSalt` is wired from the `audit-hash-salt` Key Vault secret in Bicep; see `docs/security-architecture.md`. | Medium — Cosmos diagnostic logs capture operation type and resource but not full document diff. If the operator has not populated `audit-hash-salt`, runtime falls back to plaintext (fail-open). |
