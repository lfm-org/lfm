...
| **Repudiation** | Attacker denies having logged in or disputes which account authenticated. | `AuditLog.Emit` writes `login.success` and `login.failure` events including the actor id; `IActorHasher` HMAC-hashes the id when `AuditOptions.HashSalt` is configured (production). `Audit__HashSalt` is wired in `infra/modules/functions.bicep` from the `audit-hash-salt` Key Vault secret — see `docs/security-architecture.md` for the deploy-prerequisite list. App Insights + Log Analytics retain the structured events. | Medium — when the salt is configured, attribution requires reproducing the HMAC; if the salt is rotated or lost, historical events become permanently unlinkable to real Battle.net accounts. If the operator has **not** populated `audit-hash-salt`, the runtime falls back to plaintext logging — fail-open mode. |
