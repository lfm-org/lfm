...
test("local test-mode login redirects an already-configured raider to the requested route", async ({ page }) => {
  ...
  await expect(loginLink).toHaveAttribute("href", "/api/battlenet/login?redirect=%2Fruns%2Fnew");
...
test("local test-mode login routes raiders without a selected character through character selection", async ({ page }) => {
  await page.goto("/api/battlenet/login?redirect=%2Fruns%2Fnew&testAuthScenario=needs-character");

  await expect(page).toHaveURL(/\/characters\?redirect=%2Fruns%2Fnew$/);
...
test("logout clears the session and protects runs again", async ({ page }) => {
